...
def assessment_structures(request):       # 1. Assessment Structures all
    ...
    return render(request, 'assessment_structures/assessment_structures_all.html',{"assessment_structures_to_show": assessment_structures_to_show})

@has_permission_required('w_assessment_structure')
def assessment_structure_add(request):    # 2. Add a new  Assessment Structure
    if request.method == "POST":
        assessment_structure_name = Validation.validate_name(request.POST.get('as_name'))
        assessment_structure_description = request.POST.get('as_description')  
        
        new_as = Assessment_Structure(name=assessment_structure_name,description=assessment_structure_description)
        new_as.save()
...
@has_permission_required('r_assessment_structure')
def assessment_structure_edit(request,assessment_structure_id):    # 3. view/edit Assessment Structure
        ...
        if request.method == "POST":
            ...
            if request.POST.get('af_for'):
                ...
                return redirect('assessment_structure_edit', assessment_structure_id=assessment_structure.id)
            
            elif request.POST.get('as_name'):
                assessment_structure.name = Validation.validate_name(request.POST.get('as_name'))
            ...
            return render(request, 'assessment_structures/assessment_structure_edit.html', {"assessment_structure": assessment_structure})
...
def assessment_structure_delete(request): # 4. delete assessment structure    
    ...
    assessment_structure = Validation.validate_object(Assessment_Structure, assessment_structure_id)
    
    # Delete all the images in the ckeditor related to this assessment structure
    matching_ck_img_objects = CKImageUpload.objects.filter(for_model='Assessment_Structure', model_id=assessment_structure_id)
    for img in matching_ck_img_objects:
        img.image.delete()
        img.delete()
    
    assessment_structure.delete()
    add_activity(request,"Assessment Structure Deleted : " + str(assessment_structure.name))
    return redirect('assessment_structures')
...
def assessment_structure_field_edit(request, assessment_structure_id, f_for,key):
    ...
    if request.method == "POST":
        ...
        return render(request, 'assessment_structures/assessment_structure_field_edit.html', {"assessment_structure": assessment_structure,
                                                                                              'f_for': f_for,
                                                                                              'key_to_edit': key})
