...
# Handel html to subdoc
def fix_html_img(html_content):
    soup = BeautifulSoup(html_content, 'html.parser')
        
    # Add width to inline images
    img_tags = soup.find_all('img', class_='image_resized')
    for img_tag in img_tags:
        ...
        for part in style_parts:
            if 'width:' in part:
                width_percent = float(part.replace('width:', '').replace('%', '').strip())
                break
        if width_percent is not None:
            img_tag['width'] = str(int(width_percent))
    
    # Add width to figures
    # figure_tags = soup.find_all('figure', class_='image_resized')
    figure_tags = soup.find_all('figure', class_='image')
    for figure_tag in figure_tags:
        ...
        else: # no resized imges found give them width of 75%
            width_value = '72'
            img_tag = figure_tag.find('img')
            if img_tag:
                img_tag['width'] = width_value
...
def html_2_sub_docx(html) :
    """ get html string and return --> a docmunet in memeory  """
    html = fix_html_img(html)

    # cheak if the html is text only (start with text++) when yes return text
    soup = BeautifulSoup(html, 'html.parser')
    only_p_tags = all(tag.name == 'p' for tag in soup.find_all())
    ...
    parser.add_html_to_document(html , desc_document)
    
    # Remvoe justification from the tables and alighn left
    for table in desc_document.tables:
        for row in table.rows:
            for cell in row.cells:
                for paragraph in cell.paragraphs:
                    paragraph.alignment = WD_PARAGRAPH_ALIGNMENT.LEFT
    
    docx_in_memory = io.BytesIO()
    desc_document.save(docx_in_memory)
    docx_in_memory.seek(0)
    return docx_in_memory

# -------------- end of FAROUK FUNCTIONS --------------
...
def fetch_image_resize(url, percent):
    try:
        with urllib.request.urlopen(url) as response:
            ...
            # Resize the image to the calculated dimensions with the ANTIALIAS filter
            resized_image = image.resize((new_width, new_height), Image.ANTIALIAS)

            # Convert the resized image back to bytes
            output_data = io.BytesIO()
            resized_image.save(output_data, format=image.format)

            return output_data
    except urllib.error.URLError:
        return None

def fetch_image_with_dimensions(url, percent):
    try:
        with urllib.request.urlopen(url) as response:
            image_data = io.BytesIO(response.read())
            image = Image.open(image_data)
            ...
            width_inches = new_width / 96.0  # Assuming 96 pixels per inch
            height_inches = new_height / 96.0
            return image_data, width_inches, height_inches
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag in ['p', 'pre']:
            ...
            if isinstance(self.doc, docx.document.Document):
                ...
                # the negtive one (-1) below is from farouk hehehe
                h_size = int(tag[1]) - 1
