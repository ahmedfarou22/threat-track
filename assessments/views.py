from .models import Assessment,AssessmentStatus, AssessmentFile, AssessmentVulnerability, TaskStatus
...
def assessments(request):
    ...
    if selected_user:
        assessments_list = assessments_list.filter(assigned_users__username=selected_user)
    ...
    return render(request, 'assessments/assessments_all.html', {
        'assessments': assessments,
        'clients': Client.objects.filter(assessments__in=assessments_list2).distinct(),
        'users': User.objects.filter(assigned_assessments__in=assessments_list2).distinct(),
...
def assessment_add(request):
    if request.method == 'POST':
        ...
        assessment = Assessment.objects.create(name=name,client=client, status=status,start_date = start_date,end_date = end_date,who_created=request.user)
        assessment.assigned_users.set(Validation.validate_users(request.POST.getlist('assigned_users'))) if request.POST.getlist('assigned_users') else None
        
        if request.POST['assessment_structure_id'] != "0":
            assessment_structure = Validation.validate_object(Assessment_Structure,request.POST['assessment_structure_id'])
        ...
        return render(request, 'assessments/assessment_add.html', {'clients':clients,
...
def assessment_edit(request,assessment_id):
    ...
    user_permissions = request.user.userprofile.role.permissions.values_list('name', flat=True)
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        if request.method == 'POST':
            assessment.name = Validation.validate_name(request.POST['name'])
            assessment.status = Validation.validate_object(AssessmentStatus,request.POST['status_id'])
            assessment.client = Validation.validate_object(Client,request.POST['client'])
            assessment.assigned_users.set(Validation.validate_users(request.POST.getlist('assigned_users')))
            ...
            if datetime.strptime(request.POST['start_date'], '%Y-%m-%d') > datetime.strptime(request.POST['end_date'], '%Y-%m-%d'):
                raise BadRequest('Start Date must be before the end date')
            
            if request.POST['assessment_structure_id'] != "0": # Changing the assessment struckture
                assessment_structure = Validation.validate_object(Assessment_Structure,request.POST['assessment_structure_id'])
                if assessment_structure.name != assessment.af_name: # cheak if the assessment struckture is changed
                    assessment.af_name  = assessment_structure.name
                    assessment.s_fields = assessment_structure.s_fields
                    assessment.a_fields = assessment_structure.a_fields
                    assessment.v_fields = assessment_structure.v_fields
            ...
            elif request.POST['assessment_structure_id'] == "0": # No assessment struckture
            ...
            return HttpResponseRedirect(reverse('assessment_summary', args=[assessment_id]))
            ...
            teams = Team.objects.all()
            users = User.objects.all()
            return render(request, 'assessments/assessment_edit.html', {"assessment": assessment,
                                                                        'clients':clients,
                                                                        'assessment_structures':assessment_structures,
                                                                        'statuses':statuses,
                                                                        'teams':teams,
...
def assessment_delete(request):
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        ...
        matching_ck_img_objects = CKImageUpload.objects.filter(for_model='Assessment', model_id=assessment_id)
        ...
        for vuln in assessment.vulnerabilities.all():
            matching_ck_img_objects = CKImageUpload.objects.filter(for_model='AssessmentVulnerability', model_id=vuln.id)
            for img in matching_ck_img_objects:
                img.image.delete()
                img.delete()
...
def assessment_summary(request,assessment_id):
    ...
    user_permissions = request.user.userprofile.role.permissions.values_list('name', flat=True)
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        return render(request, 'assessments/assessment_summary.html', {"assessment": assessment})
    else:
        raise PermissionDenied
...
def assessment_additional_fields(request,assessment_id):
    ...
    user_permissions = request.user.userprofile.role.permissions.values_list('name', flat=True)
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        return render(request, 'assessments/assessment_additional_fields.html', {"assessment": assessment, 'fields': Assessment_Structure.objects.all()})
    else:
        raise PermissionDenied
...
def assessment_vulnerabilities(request,assessment_id):
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        ...
        if selected_risk:
            vulnerabilities_list = vulnerabilities_list.filter(risk_rating=selected_risk)
        ...
        context = {"assessment": assessment,"vulnerabilities":vulnerabilities_list, "selected_risk":selected_risk,"selected_status":selected_status,
                   "total_vulnerabilities":len(total_vulnerabilities),"vulnerabilities_list":len(vulnerabilities_list),}
...
def assessment_attached_files(request,assessment_id):
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        if request.method == 'GET':
            return render(request, 'assessments/assessment_attached_files.html', {"assessment": assessment})
            ...
            if 'w_assessment_attached_files' not in user_permissions:
                ...
                if file:
                    if file.size > 20 * 1024 * 1024:
                        messages.error(request,"File size exceeds the allowed limit of 20MB")
                        return redirect('assessment_attached_files', assessment_id=assessment.id)
                    
                    new_file = AssessmentFile(name=file.name, file=file, added_by=request.user)
                    new_file.save()

                    # Update metadata of files in S3 or Blob
                    update_image_metadata(new_file.file.name) if settings.MEDIA_STORAGE_TYPE in ['S3', 'BLOB'] else None
...
def assessment_tasks(request,assessment_id):
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        ...
        return render(request, 'assessments/assessment_tasks.html',{"assessment": assessment,
        ...
        if request.method == "POST":
            if 'w_assessment_chat_room' not in user_permissions:
                ...
                if request.POST.get('message'):
                    new_message = ChatMessage(assessment=assessment,sender=request.user,message=request.POST.get('message'))
                    new_message.save()
            ...
            return render(request, 'assessments/assessment_chat_room.html', {"assessment": assessment,
                                                                            'messages_to_view': messages_to_view})
    else:
        raise PermissionDenied
...
def assessment_reporting(request,assessment_id):
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        if request.method == "POST":
            if request.POST.get('template_from_comp'):
                template_obj = Validation.validate_object(Template, request.POST.get('template_from_comp'))
                ...
                uploaded_file = Validation.validate_docxFile(request.FILES['template_from_upload'])
                
                new_template_object = Template(name="tmp",file=uploaded_file, chart_settings = json.loads("""{"charts": [{"type": "pie","title": "Vulnerability Risk Distribution (Pie)","x_label": "","y_label": "","size": 120,"colors": {"Critical": "#c00000","High": "#ff0000","Medium": "#ffc000","Low": "#ffff00"}},{"type": "bar","title": "Vulnerability Risk Distribution (Bar)","x_label": "Risk Levels","y_label": "Count","size": 120,"colors": {"Critical": "#c00000","High": "#ff0000","Medium": "#ffc000","Low": "#ffff00"}}]} """))
                
                try:
                    genrated_report =  genrate_report(new_template_object,assessment_dict)
            ...
            return render(request, 'assessments/assessment_reporting.html',{"assessment": assessment, "assessment_dict":assessment_dict,
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        if request.method == 'POST':
            ...
            new_vulnerability = AssessmentVulnerability(
                ...
                status=Validation.validate_vuln_status(request.POST.get('v_status')), # TODO make sure it is on of 2
                cvss=Validation.validate_cvss(request.POST.get('v_cvss')) or 0,
                risk_rating=Validation.validate_risk_rating(request.POST.get('v_risk_rating')),
            ...
            matching_ck_img_objects = CKImageUpload.objects.filter(for_model='AssessmentVulnerability', model_id=0)
            for img in matching_ck_img_objects:
                img.model_id = new_vulnerability.id
                img.save()
            
            add_activity(request,"Vulnerability Added to assessment: " + str(new_vulnerability.name))
            return redirect('assessment_vulnerabilities', assessment_id=assessment.id)

        elif request.method == 'GET':
            if not request.GET.get('vuln_to_load_id'):
                return render(request, 'assessments/assessment_vulnerabilities_add.html', {"assessment": assessment,'com_vuln':Vulnerability.objects.all()})
            
            else:
                vuln_to_load = get_object_or_404(Vulnerability, pk=request.GET.get('vuln_to_load_id'))
                response_data = {
            ...
            if scan_type == "nessuss":
                ...
                for row in reader:
                    if len(row) >= 10:
                        ...
                        if cvss_score:
                            new_vulnerability = AssessmentVulnerability(assessment=assessment,status="Unresolved", target=host + ":" + port + " (" + protocol.upper() + ")", name=name, description=description, tag=cve, cvss=cvss_score or 0, risk_rating=risk, remediation=solution, fields=assessment.v_fields)
                            new_vulnerability.save()
                            assessment.vulnerabilities.add(new_vulnerability)
...
def assessment_vulnerability_edit(request,assessment_id,vulnerability_id):
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        if request.method == 'GET':
            return render(request, 'assessments/assessment_vulnerability_edit.html', {"assessment": assessment,'assessment_vulnerability':assessment_vulnerability})
    
        elif request.method == 'POST':
            if 'w_assessment_vulnerabilities' not in user_permissions:
                raise PermissionDenied
        
            assessment_vulnerability.name = Validation.validate_name(request.POST.get('v_name'))
            assessment_vulnerability.status = Validation.validate_vuln_status(request.POST.get('v_status'))
            assessment_vulnerability.cvss = Validation.validate_cvss(request.POST.get('v_cvss')) if request.POST.get('v_cvss') else 0
            assessment_vulnerability.risk_rating = Validation.validate_risk_rating(request.POST.get('v_risk_rating'))
            ...
            if request.POST.get('edit') == "1": # if the user clicked edit just edit
                add_activity(request,"Assessment Vulnerability Edited : " + str(assessment_vulnerability.name) + "at " + str(assessment.name))
                return redirect('assessment_vulnerabilities', assessment_id=assessment_id)
            
            elif request.POST.get('render_short_codes') == "1": # the user cicked on render short codes
                try:
                    assessment_dict = all_in_one(assessment)
                    assessment_vulnerability.name = Validation.validate_name(render_short_codes(request.POST.get('v_name'),assessment_dict))
                    assessment_vulnerability.status = Validation.validate_vuln_status(request.POST.get('v_status'))
                    assessment_vulnerability.cvss = Validation.validate_cvss(request.POST.get('v_cvss')) if request.POST.get('v_cvss') else 0
                    assessment_vulnerability.risk_rating = Validation.validate_risk_rating(request.POST.get('v_risk_rating'))
                    
                    assessment_vulnerability.target = render_short_codes(request.POST.get('v_target'),assessment_dict)
                    assessment_vulnerability.tag = render_short_codes(request.POST.get('v_tag'),assessment_dict)
                    assessment_vulnerability.description = render_short_codes(request.POST.get('v_description'),assessment_dict)
                    assessment_vulnerability.impact = render_short_codes(request.POST.get('v_impact'),assessment_dict)
                    assessment_vulnerability.remediation = render_short_codes(request.POST.get('v_remediation'),assessment_dict)           
                    assessment_vulnerability.poc_text = render_short_codes(request.POST.get('v_poc_text'),assessment_dict)
                    
                    if assessment_vulnerability.fields:
                        for v_filed in assessment_vulnerability.fields:
                            assessment_vulnerability.fields[v_filed] = render_short_codes(request.POST.get(v_filed),assessment_dict)
                    
                    assessment_vulnerability.save()
                    messages.success(request, "Shortcodes rendered successfully")
                    return redirect('assessment_vulnerability_edit',assessment_id=assessment.id ,vulnerability_id=assessment_vulnerability.id )
...
def assessment_vulnerability_delete(request):
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        ...
        matching_ck_img_objects = CKImageUpload.objects.filter(for_model='AssessmentVulnerability', model_id=vulnerability.id)
        for img in matching_ck_img_objects:
            img.image.delete()
            img.delete()
...
def add_task(request, assessment_id):
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        if request.method == 'POST':
            ...
            status =  Validation.validate_object(TaskStatus, int(request.POST['f_status']))
            assigned_users = Validation.validate_users(request.POST.getlist('assigned_users'))
            ...
            for user in assigned_users_objects: 
                if user not in assessment.assigned_users.all():
                    raise BadRequest("You can only assign users who are part of the assessment")
            ...
            return render(request, 'assessments/assessment_task_add.html',{"assessment": assessment,
                                                                        'statuss_to_load': statuss_to_load,
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        if request.method == 'POST':
            ...
            if 'f_status' in request.POST:
                task.status = Validation.validate_object(TaskStatus, int(request.POST['f_status']))
            ...
            assigned_users = assessment.assigned_users.all()
            task_to_edit = task
            return render(request, 'assessments/assessment_task_edit.html',{"assessment": assessment,
                                                                        'statuss_to_load': statuss_to_load,
                                                                        'assigned_users':assigned_users,
...
def assessment_field_edit(request,assessment_id,f_for,key): 
    ...
    if request.user in assessment.assigned_users.all() or 'view_all_assessments' in user_permissions:
        if request.method == 'POST':
            ...
            return render(request, 'assessments/assessment_additional_field_edit.html', {"assessment": assessment,
                                                                                         "key_to_edit":key,
                                                                                         'f_for':f_for})
            ...
            try:
                if f_for == "s":
                    if 'w_assessment_summary' not in user_permissions:
                        raise PermissionDenied
                    if key in assessment.s_fields:
                        assessment.s_fields[key] = render_short_codes(assessment.s_fields[key],assessment_dict) 
                    assessment.save()
                    messages.success(request, "Shortcodes rendered successfully for " + str(key))
                    return redirect('assessment_summary', assessment_id=assessment.id)
                    ...
                    if 'w_assessment_additional_fields' not in user_permissions:
                        raise PermissionDenied
                    if key in assessment.a_fields:
                        assessment.a_fields[key] = render_short_codes(assessment.a_fields[key],assessment_dict) 
                    assessment.save()
                    messages.success(request, "Shortcodes rendered successfully for " + str(key))
                    return redirect('assessment_additional_fields', assessment_id=assessment.id) 
